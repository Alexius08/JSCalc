...
document.getElementById("removecurrent").addEventListener("click", function () {
  if (!hasError)
  clearcurrent();
});
...
document.getElementById("clearall").addEventListener("click", function () {
  clearcurrent();
  Array.from(operators, function (o) {return o.style.backgroundColor = "#afa";});
  num[1] = {
    digits: undefined,
    decPoint: 0,
    hasDecPoint: false };
...
document.getElementById("changesign").addEventListener("click", function () {
  if (!hasError) {
    if (!canGetNextNum) {
      ...
      screen.innerHTML = num[0].hasDecPoint ? num[0].digits / Math.pow(10, num[0].decPoint).toFixed(num[0].decPoint) : num[0].digits;
    }
  }
});
...
Array.from(operators, function (o) {return o.addEventListener("click", operate);});
...
Array.from(numbers, function (d) {return d.addEventListener("click", digit);});
...
function calcAnswer() {
  try {
    switch (selectedOp) {
      ...
      case "add":
        ...
        } else
        {
          ...
          num[0].digits *= Math.pow(10, num[1].decPoint - num[0].decPoint);
          num[1].digits += num[0].digits;
      ...
      case "divide":
        ...
        } else
        {
          if (num[1].decPoint < 0) {
            num[1] = {
              digits: round(num[1].digits * Math.pow(10, -num[1].decPoint), 15),
              decPoint: 0 };
    ...
    if (Math.abs(num[1].digits / Math.pow(10, num[1].decPoint)) > Math.pow(10, 14))
    throw "Out of range";else
    if (num[1].decPoint > 0) {
      var extraDigits = 0;
      ...
      if (num[1].decPoint > 14) {
        num[1] = {
          digits: Math.round(num[1].digits / (Math.pow(10, num[1].decPoint) - 14)),
  ...
  if (!hasError) {
    screen.innerHTML = num[1].hasDecPoint ? (num[1].digits / Math.pow(10, num[1].decPoint)).toFixed(num[1].decPoint) : num[1].digits;
  }
}
